...
Για να τρέξει η εφαρμογή πρέπει πρώτα να εγκατασταθούν οι βιβλιοθήκες που απαιτείται από το αρχείο requirements.txt και κατόπιν να εκτελεστεί το αρχείο "create_admin.py" .Με αυτό δημιουργείται ένας "ADMINISTRATOR" account  με username : "admin" και password : "iee2019240" . Εαν επιθυμεί ο προγραμματιστής διαφορετικά στοιχεία θα πρέπει να τροποποιήσει το αρχείο κατάλληλα στις γραμμές 14 & 15  και ΜΕΤΑ να το εκτελέσει. Με την σωστή εκτέλεση χωρίς σφάλματα θα πάρει μήνυμα επιβεβαίωσης.

Για την εκτέλεση του ΚΥΡΙΟΥ προγράμματος θα πρέπει να εκτελεστεί ως modular πρόγραμμα - με παράμετρο "-m". 
